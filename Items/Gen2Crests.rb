...
DONPHAN_CREST = CrestBuilder.add(:DONPHAN, "Added Fighting-type. 1/6 end-of-turn recovery when hit by a physical move.")
            ...
            .on_turn_end { |pkmn|
              if pkmn.effects[:DonphanCrest] and pkmn.effects[:HealBlock] == 0 and
                pkmn.pbRecoverHP((pkmn.totalhp / 6).floor, true)
                UniLib.display_if_visible(pkmn.battle, _INTL("{1}'s {2} restored its HP a little!", pkmn.pbThis, pkmn.item.name))
              end
