RemiX.chapter(5) {
  ...
  BossBuilder.create(:RAT_FAMILY, :RATICATE, 42, :STRONGJAW, form: 1, ev: ATKSPE, nature: :JOLLY, moves: [:CRUNCH, :HYPERFANG, :FIREFANG, :SUCKERPUNCH])
             ...
             .set_sos_count(3)
             .set_sos_pkmn(1, :RATICATE, 40, :RUNAWAY, item: RATICATE_CREST, moves: [:SUPERFANG, :FORESIGHT], ev: HPSPE, nature: :JOLLY)

}
RemiX.chapter(6) {

  BossBuilder.create(:LAPRAS_BOSS, :LAPRAS, 50, :NOGUARD, item: LAPRAS_CREST, ev: HPSPA, nature: :TIMID, moves: [:HYDROPUMP, :ICEBEAM, :THUNDER, :SAFEGUARD])
             .set_shields(4)
             .set_name("Nessie")
...
RemiX.chapter(8) {
  ...
  BossBuilder.create(:TORKOAL_BOSS, :TORKOAL, 60, [:DROUGHT, :SOLIDROCK], item: TORKOAL_CREST, ev: HPSPA, nature: :MODEST, moves: [:LAVAPLUME, :SCALD, :EARTHPOWER, :TRICKROOM])
             .set_shields(4)
